...
import { SPECIALTY_LIST_SELECT, type SpecialtyRow } from "@/server/specialties/service";
import {
  buildQueryHref,
  readPageParam,
  readStringParam,
...
export default async function SpecialtiesPage({
  ...
  const where: Prisma.SpecialtyWhereInput = query
    ? {
        OR: [
  ...
  return (
    <AdminPage>
      <AdminPageHeader
        title="Specialties"
        description="Clinical departments the company equips. A category is what the equipment is; a specialty is who uses it."
        actions={
          can("SPECIALTIES", "CREATE") ? (
            <Link href="/admin/specialties/new" className={buttonStyles()}>
      ...
      <DataTable<SpecialtyRow>
        rows={rows}
        rowId={(row) => row.id}
        rowLabel={(row) => row.name}
        rowHref={(row) => `/admin/specialties/${row.id}`}
        ...
        emptyState={
          <EmptyState
            title={query ? "No matching specialties" : "No specialties yet"}
            description={
              query
                ? "Try a different name or slug."
                : "Add the clinical departments the company equips."
            }
            action={
              query ? (
                <Link
                  href={buildQueryHref("/admin/specialties", {}, {})}
